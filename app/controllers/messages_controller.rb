	...
	def create
		...
		if chat_room != nil && correct_user(chat_room)
			...
			if current_user.id == Room.find(params[:room_id]).seller_id
				Notification.create(recipient: chat_room.buyer, actor: current_user, action: "sent", notifiable: message)
			else
				Notification.create(recipient: chat_room.seller, actor: current_user, action: "sent", notifiable: message)
			end
